...
public class PaimentController {
    ...
    @PostMapping("/addpaiement/{nbr}")
    public ResponseEntity<Paiement> addModePaiement(@RequestBody Paiement modePaiement,@PathVariable("nbr") Integer nbr) {
        // Call the service layer to add a paiement
        Paiement addedModePaiement = modePaiementService.addModePaiement(modePaiement,nbr);
